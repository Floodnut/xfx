...
**왜 ioctl 기반 `/dev/liveupdate`인가**: LWN 기사에 따르면 v2 개정에서 sysfs 기반 제어를 ioctl 기반으로 바꿨다. sysfs는 "파일 하나 읽고 쓰기"엔 편하지만, 세션 생성·복수 파일 등록·상태 전이처럼 여러 단계로 이어지는 트랜잭션성 작업엔 원자성을 보장하기 어렵다 — ioctl은 커널이 요청 하나를 통째로 처리할 수 있어 이런 워크플로우에 더 자연스럽다.
